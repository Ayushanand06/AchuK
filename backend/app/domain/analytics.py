...
class AnalyticsEngine:
    ...
    def patrol_recommendations(self) -> List[PatrolRecommendation]:
        ...
        for zone, hour_counts in zone_hour.items():
            ...
            recs.append(PatrolRecommendation(
                zone, priority, reason, f"{ws}–{we}", units,
                expected=predicted, multiplier=round(ratio, 1),
            ))
    ...
    def zone_hour_matrix(self, days: int = 7) -> dict:
        """
        Zone × 24-hour violation count matrix for the heatmap.
        { "zones": [...], "matrix": [[h0..h23], ...], "max": int }
        Zones ordered by total volume (busiest first).
        """
        records = self._load_records(days)
        zones: Dict[str, List[int]] = {}
        for r in records:
            zone = r.get("camera_location", "Unknown") or "Unknown"
            try:
                h = datetime.fromisoformat(r["timestamp"].rstrip("Z")).hour
            except Exception:
                continue
            zones.setdefault(zone, [0] * 24)[h] += 1

        ordered = sorted(zones.keys(), key=lambda z: sum(zones[z]), reverse=True)
        matrix = [zones[z] for z in ordered]
    ...
    def buckets(self, days: int = 7) -> dict:
        """
        Counts + average CVCS per decision bucket from stored records.
        'discarded' is not stored (discards never create a challan), so it is
        reported as not-tracked.
        """
        records = self._load_records(days)

        def summarise(decision: str) -> dict:
            rs = [r for r in records if r.get("cvcs_decision") == decision]
            avg = sum(r.get("cvcs_score", 0) for r in rs) / len(rs) if rs else 0.0
            return {"count": len(rs), "avg_cvcs": round(avg, 3)}

        return {
            "auto": summarise("auto_challan"),
            "review": summarise("review"),
            "discarded": {"count": None, "avg_cvcs": None, "tracked": False},
            "total": len(records),
        }
